...
fn main() -> Result<()> {
    ...
    println!(
        "Part 1: {:?}",
        intcode_machine.proceed_until_halt(VecDeque::from([1])).0[0]
    );

    let mut intcode_machine = IntCodeMachine::new(instructions);
    println!(
        "Part 2: {:?}",
        intcode_machine.proceed_until_halt(VecDeque::from([2])).0[0]
    );
